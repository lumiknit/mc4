...
public class OzHuman : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(!lassitude) {
            UpdateTargetPos();
            UpdateArmPos(handL, shoulderL, lOff);
        ...
        if(boat != null) {
            ...
            if(ex.y < py) {
                ...
                boat.GetComponent<Rigidbody>().AddForceAtPosition(-dx.normalized * 5 * dl * dl * Mathf.Abs(ex.y - py), point, ForceMode.VelocityChange);
                Debug.Log("aa");
                boat.GetComponent<Rigidbody>().velocity *= 0.98f;
            }
            oarEndLastPos = ex;
            footL.GetComponent<Rigidbody>().velocity += new Vector3(0f, -2f, 0f);
            footR.GetComponent<Rigidbody>().velocity += new Vector3(0f, -2f, 0f);
        }
    }
    ...
    private void UpdateArmPos(GameObject hand, GameObject shoulder, Vector3 targetOffset) {
        ...
        rHand.velocity = GetComponent<Rigidbody>().velocity + d * m + a * 2f;
    }
    ...
    public class TestHSwing1 : Action {
        ...
        public override void Step(float t, float dt) {
            ...
            float x;
            if(!float.IsNaN(x = TRange(prog, 0.0f, 0.2f))) {
                oz.txa = 120f;
                oz.tya = -10f;
                oz.tra = 1f;
                oz.lOff = new Vector3(0, 0, 0);
                oz.armSpd = 20f;
            } else if(!float.IsNaN(x = TRange(prog, 0.2f, 0.3f))) {
                oz.txa = 0f;
                oz.tya = -10f;
                oz.tra = 1f;
                oz.lOff = new Vector3(0.8f, 0.0f, 0.3f);
                oz.armSpd = 20f;
            } else if(!float.IsNaN(x = TRange(prog, 0.3f, 0.5f))) {
                oz.txa = -120f;
                oz.tya = -10f;
                oz.tra = 1f;
                oz.lOff = new Vector3(1.1f, 0.0f, 0.3f);
                oz.armSpd = 20f;
            } else {
                oz.txa = 0f;
                oz.tya = 0f;
    ...
    public class TestHSwing2 : Action {
        public const float DURATION = 1f;
        public TestHSwing2(OzHuman oz) : base(oz, DURATION) {}
        public override void Step(float t, float dt) {
            float prog = t / DURATION;
            float x;
            if(!float.IsNaN(x = TRange(prog, 0.0f, 0.06f))) {
                oz.txa = 120f;
                oz.tya = 0f;
                oz.tra = 1f;
            } else if(!float.IsNaN(x = TRange(prog, 0.06f, 0.12f))) {
                oz.txa = 0f;
                oz.tya = 0f;
                oz.tra = 1f;
            } else if(!float.IsNaN(x = TRange(prog, 0.12f, 0.20f))) {
                oz.txa = -60f;
                oz.tya = 0f;
                oz.tra = 1f;
            } else if(!float.IsNaN(x = TRange(prog, 0.20f, 0.3f))) {
                oz.txa = -90f;
                oz.tya = 0f;
                oz.tra = 1f;
            } else if(!float.IsNaN(x = TRange(prog, 0.3f, 0.4f))) {
                oz.txa = -120f;
                oz.tya = 0f;
                oz.tra = 1f;
            } else {
    ...
    public class TestRow1 : Action {
        public const float DURATION = 1.8f;
        public TestRow1(OzHuman oz) : base(oz, DURATION) {}
        public override void Step(float t, float dt) {
            float prog = t / DURATION;
            float x;
    ...
    public class TestRow3 : Action {
        public const float DURATION = 1.8f;
        public TestRow3(OzHuman oz) : base(oz, DURATION) {}
        public override void Step(float t, float dt) {
            float prog = t / DURATION;
            float x;
            if(!float.IsNaN(x = TRange(prog, 0.0f, 0.125f))) {
    ...
    public class TestRow2 : Action {
        public const float DURATION = 1.5f;
        public TestRow2(OzHuman oz) : base(oz, DURATION) {}
        public override void Step(float t, float dt) {
            float prog = t / DURATION;
            float x;
