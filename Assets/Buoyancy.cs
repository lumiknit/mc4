...
public class Buoyancy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (transform.position.y < nearestVert.y)
        {    //If this object is below the nearest vertice
    ...
    Vector3 NearestVertice(Vector3 pos, Vector3[] verts)            //Takes a position and a position array
    {
        ...
        {        //For every vertice
            if (Vector3.Distance(pos, verts[i]) < minDist)
            {        //If the vertice is closer than the one before it
